...
|===
|Area |Task |Model |Dataset |QSL Size |Quality |Server latency constraint | Minimum network bandwidth (bytes/sec)
|Vision |Image classification |Resnet50-v1.5 |ImageNet (224x224) | 1024 | 99% of FP32 (76.46%) | 15 ms | throughput x 150528
|Vision |Object detection (large) |SSD-ResNet34 |COCO (1200x1200) | 64 | 99% of FP32 (0.20 mAP) | 100 ms | throughput x 270000
|Vision |Medical image segmentation |3D UNET |BraTS 2019 (224x224x160) | 16 | 99% of FP32 and 99.9% of FP32 (0.85300 mean DICE score) | N/A | throughput x 32381026
|Speech |Speech-to-text |RNNT |Librispeech dev-clean (samples < 15 seconds) | 2513 | 99% of FP32 (1 - WER, where WER=7.452253714852645%) | 1000 ms | throughput x 1323000
|Language |Language processing |BERT |SQuAD v1.1 (max_seq_len=384) | 10833 | 99% of FP32 and 99.9% of FP32 (f1_score=90.874%) | 130 ms | throughput x 4608
|Commerce |Recommendation |DLRM |1TB Click Logs | 204800 |99% of FP32 and 99.9% of FP32 (AUC=80.25%) | 30 ms |  throughput x 32400
|===
...
The average input size (in bytes) for each model are given by the following equations.
...
[listing,indent=0]
----
input size  = avg(C*D*H*W)
            = 32381026
----

The inference cases for 3D UNET are specified in https://github.com/mlcommons/inference/blob/master/vision/medical_imaging/3d-unet-kits19/meta/inference_cases.json[inference_cases.json]. The average image size above is the average of the image sizes in those inference cases.

==== RNNT

[listing,indent=0]
----
input size  = max_audio_duration*num_samples_per_sec*sizeof(float16)
            = 5*4410*2
            = 1323000
----
...
[listing,indent=0]
----
input size  = num_inputs*max_seq_len*sizeof(int32)
            = 3*384*4
            = 4608
----
...
[listing,indent=0]
----
input size  = avg(num_pairs_per_sample)*(num_numerical_inputs*sizeof(int8) + num_categorical_inputs*sizeof(int32))
            = 270*(16+26*4)
            = 32400
----

Each DLRM sample consists of up to 700 user-item pairs draw from the distribution specified in https://github.com/mlcommons/inference/blob/master/recommendation/dlrm/pytorch/tools/dist_quantile.txt[dist_quantile.txt].
